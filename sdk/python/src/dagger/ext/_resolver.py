...
class Resolver(ABC, Generic[T]):
    ...
    def __post_init__(self):
        ...
        try:
            orig_base = self.__orig_bases__[0]  # type: ignore reportGeneralTypeIssues
            allowed = get_args(orig_base)[0]
        except (AttributeError, IndexError) as e:
            msg = f"Unable to get allowed return types for {type(self).__name__}."
            raise InternalError(msg) from e

        # If not a union of values, assume single value.
        allowed = get_args(allowed) or (allowed,)

        if self.return_type not in allowed:
            msg = (
                f"Invalid return type “{self.return_type}”. "
    ...
    def to_decorator(cls, mod: Module):
        ...
        deco.__module__ = mod.__module__
    ...
    def return_type(self):
        ...
        try:
            ...
            # Assume None if no return type is specified.
            return NoneType
    ...
    @abstractmethod
    def register(self, mod: dagger.Environment) -> dagger.Environment:
        """Add a new resource to current module.
